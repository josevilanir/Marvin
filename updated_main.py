        ...
        if comando:
            if "Marvin" in comando:
                ...
                if pesquisa:
                    ...
                    responde_voz(
                        "Desculpe, não consegui ouvir o que você deseja pesquisar.")
                ...
                if pesquisa:
                    ...
                    responde_voz(
                        "Desculpe, não consegui ouvir o que você deseja ouvir.")
                ...
                responde_voz(
                    "Qual música você gostaria de adicionar à playlist?")
                musica = ouvir_comando_completo()
                if musica:
                    responde_voz(
                        "Qual playlist você deseja adicionar a música?")
                    escolha_playlist = ouvir_comando_completo()
                    if escolha_playlist:
                        escolha_numero = numero_por_extenso_para_numero(
                            escolha_playlist)
                        if isinstance(
                                escolha_numero,
                                int) and 1 <= escolha_numero <= len(playlists):
                            playlist_id = playlists[escolha_numero - 1]['id']
                        else:
                            playlist_id = next(
                                (pl['id'] for pl in playlists if pl['name'].lower() == escolha_playlist.lower()),
                                None)

                        if playlist_id:
                            # Encontrar a URI da música
                            resultados = sp.search(
                                q=musica, type='track', limit=1)
                            if resultados['tracks']['items']:
                                track_uri = resultados['tracks']['items'][0]['uri']
                                adicionar_musica_playlist(
                                    playlist_id, track_uri)
                                responde_voz(
                                    f"Música '{musica}' adicionada à playlist.")
                            else:
                                responde_voz(
                                    "Desculpe, não consegui encontrar a música especificada.")
                        else:
                            responde_voz(
                                "Desculpe, não consegui encontrar a playlist especificada.")
                    else:
                        responde_voz(
                            "Desculpe, não consegui ouvir a playlist que você deseja.")
                else:
                    responde_voz(
                        "Desculpe, não consegui ouvir a música que você deseja adicionar.")
                ...
                if playlists:
                    for i, playlist in enumerate(playlists):
                        print(
                            f"{i+1}. {playlist['name']} (ID: {playlist['id']})")
                ...
                if pesquisa:
                    ...
                    responde_voz(
                        "Você deseja ouvir no modo padrão ou aleatório?")
                    ...
                    try:
                        ...
                        # Se não for possível converter, mantém a pesquisa como
                        # string
                        pass
                ...
                try:
                    ...
                    musica_nome = partes[0].replace(
                        "toque a música", "").strip()
                ...
                if aplicativo:
                    ...
                    responde_voz(
                        "Desculpe, não consegui ouvir o nome do aplicativo.")
                ...
                if contato:
                    responde_voz(
                        f"O que você gostaria de dizer para {contato}?")
                    ...
                    if mensagem:
                        ...
                        responde_voz(
                            "Não consegui entender a mensagem. Por favor, tente novamente.")
                else:
                    responde_voz(
                        "Não consegui entender o nome do contato. Por favor, tente novamente.")

            elif "Tocar" in comando and "YouTube" in comando or "tocar" in comando and "YouTube" in comando:
                pesquisa = comando.replace(
                    "tocar", "").replace(
                    "no YouTube", "").strip()
                ...
                if pesquisa:
                    ...
                    responde_voz(
                        "Pesquisa realizada para {} no YouTube.".format(pesquisa))
                ...
                titulo_video = comando.replace(
                    "clicar no Vídeo".lower(), "").strip()
