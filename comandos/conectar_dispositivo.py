    ...
    if dispositivos:
        ...
        if escolha:
            try:
                ...
                if escolha_numero is not None and 1 <= escolha_numero <= len(
                        dispositivos):
                    dispositivo_id = dispositivos[escolha_numero - 1]['id']
                else:
                    # Caso não seja um número por extenso ou fora do intervalo,
                    # tenta converter a escolha direta
                    dispositivo_id = next(
                        (d['id'] for d in dispositivos if d['name'].lower() == escolha.lower()), None)

                if dispositivo_id:
                    try:
                        sp.transfer_playback(
                            device_id=dispositivo_id, force_play=True)
                        responde_voz(f"Conectado ao dispositivo: {escolha}")
                    except SpotifyException as e:
                        responde_voz(f"Erro ao conectar ao dispositivo: {e}")
                else:
                    responde_voz(
                        "Desculpe, não consegui encontrar o dispositivo especificado.")
            except ValueError:
                responde_voz("Desculpe, não entendi a seleção do dispositivo.")
        else:
            responde_voz(
                "Desculpe, não consegui ouvir o dispositivo que você deseja conectar.")
    else:
