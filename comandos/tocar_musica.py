...
def listar_musicas_da_playlist(pesquisa):
    try:
        ...
        if numero is not None:
            ...
            playlist = next((p for p in playlists if p['name'].lower() == pesquisa.lower()), None)
            if playlist:
    ...
    if escolha_playlist:
        try:
            ...
            if escolha_numero is not None:
                ...
                playlist_id = next((p['id'] for p in playlists if p['name'].lower() == escolha_playlist.lower()), None)
...
def pausar_musica():
    ...
    if not dispositivos:
        raise Exception("Nenhum dispositivo disponível. Por favor, verifique se há dispositivos conectados.")
...
def tocar_playlist(pesquisa, modo='standard'):
    ...
    if not dispositivos:
        responde_voz("Nenhum dispositivo disponível. Por favor, verifique se há dispositivos conectados.")
        return
    
    # Verifica se há um dispositivo ativo
    playback_info = sp.current_playback()
    if not playback_info or not playback_info.get('device'):
        responde_voz("Nenhum dispositivo ativo encontrado. Por favor, selecione um dispositivo.")
        return
